...
import { downsampleSplatBytes, loadSogChunks } from './shared/scene-loader.js';
// ★ R-06: 共享统计聚合
...
export class RenderManager implements RendererAdapter {
  ...
  // ★ R-06: 当前场景 splat 数 (Spark 无公开计数, 由加载路径记录)
  private _currentSplatCount = 0;
  ...
  constructor(options: RenderManagerOptions = {}) {
    ...
    // ★ N-05: 保留原始设备分级, DPR 变化时重算 tier 参数
  ...
  start(): void {
    ...
    // ★ N-05: DPR 变化监听 — 高分屏切换/窗口跨屏时更新 pixelRatio
  ...
  async preloadScene(source: string): Promise<void> {
    if (!source) return;
    // 已预取或非 splat/spz 资源 (SOG 流式无需预取) 时跳过
  ...
  private async loadSceneWithTruncatedSplat(source: string, options?: LoadOptions): Promise<void> {
    ...
    this._currentSplatCount = totalSplats;
    ...
    // ★ TD-08: 统一使用共享 downsampleSplatBytes (均匀降采样, 保持空间覆盖均匀)
    //   ★ H4: 使用 SplatBufferPool 复用 ArrayBuffer, 减少 GC 压力;
    //   ★ D-10: 追踪发放的缓冲, 场景切换/销毁时归还池中激活复用
    const { data: sampledData } = downsampleSplatBytes(
      fullData,
      maxSplats,
      {
        acquire: (bytes) => this._bufferPool.acquire(bytes),
        track: (buffer) => this._pooledSceneBuffers.push(buffer),
      },
      '降采样加载',
    );
  ...
  private async loadSceneWithSpz(source: string, options?: LoadOptions): Promise<void> {
    ...
    this._currentSplatCount = header.numSplats;
    console.info(
  ...
  private async loadSceneWithSogFallback(lodSource: string, options?: LoadOptions): Promise<void> {
    ...
    // ★ TD-08: 统一使用共享 downsampleSplatBytes
    //   ★ D-10: 追踪发放的缓冲, 场景切换/销毁时归还池中激活复用
    const loadedSplats = Math.floor(fullData.byteLength / 32);
    const maxSplats = this.tierSettings.maxSplats;
    const { data: meshData } = downsampleSplatBytes(
      fullData,
      maxSplats,
      {
        acquire: (bytes) => this._bufferPool.acquire(bytes),
    ...
    this._currentSplatCount = metadata.numSplats;
    console.info(
  ...
  // ★ R-06: 渲染统计 (帧时间/可见数/分辨率/池命中率)
  getStats(): RenderStats {
    const pool = this._bufferPool.getStats();
    return computeRenderStats({
      smoothDt: this._smoothDt,
      visibleSplats: this._currentSplatCount,
  ...
  /**
   * ★ N-05: DPR 变化监听 — 高分屏切换/窗口跨屏缩放时更新 pixelRatio。
   *
   * 使用 matchMedia 分辨率查询 (精确) + change 事件, 避免 resize 高频触发;
   * 重算 tierSettings (cappedDpr 读取最新 devicePixelRatio) 并应用。
   */
